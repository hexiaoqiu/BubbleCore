function [tmpOrgLast, u2dOrgLast, v2dOrgLast, prsOrgLast] = getAnyStepForRaw(rawCaseDir,stepIdx)
...
%   Detailed explanation goes here
    ...
    if stepIdx > maxN
        disp('Selected Step is larger than max Step! Out!')
        tmpOrgLast = NaN;
        u2dOrgLast = NaN;
        v2dOrgLast = NaN;
        prsOrgLast = NaN;
...
%                 disp('Maybe there is next step data!')
                ...
                disp('Reach the most near time step data!')
            ...
            if timeStep-1 > 0
                ...
                disp(['The demanded step idx = ',num2str(stepIdx)]);
                disp(['Output the closest step idx = ',num2str(timeStep)]);
                successfulReachLastStep = true;
                break
            else
