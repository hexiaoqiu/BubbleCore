function [configList] = getCaseDirListRc001(rootPath)
    n = 1;

    % configList(n).caseDir{1} = ...
    % 'Ra9e2/1_1_256/';
    
    % n = n + 1;
    configList(n).caseDir{1} = ...
    'Ra3e3_r_c_0.01/00_1_iniTmp0_256/';
    configList(n).caseDir{2} = ...
    'Ra3e3_r_c_0.01/00_2_iniTmp0_1024/';
    configList(n).caseDir{3} = ...
    'Ra3e3_r_c_0.01/00_3_iniTmp0_1024/';
    configList(n).caseDir{4} = ...
    'Ra3e3_r_c_0.01/00_4_1024/';
    
    % n = n + 1;
    % configList(n).caseDir{1} = ...
    % 'Ra9e3/1_1_256/';
    
    n = n + 1;
    configList(n).caseDir{1} = ...
    'Ra3e4_r_c_0.01/00_1_iniTmp0_256/';
    configList(n).caseDir{2} = ...
    'Ra3e4_r_c_0.01/00_2_iniTmp0_1024/';
    configList(n).caseDir{3} = ...
    'Ra3e4_r_c_0.01/00_3_iniTmp0_1024/';
    configList(n).caseDir{4} = ...
    'Ra3e4_r_c_0.01/00_4_1024/';

    % n = n + 1;
    % configList(n).caseDir{1} = ...
    % 'Ra9e4/1_1_256/';
    
    n = n + 1;
    configList(n).caseDir{1} = ...
    'Ra3e5_r_c_0.01/00_1_iniTmp0_256/';
    configList(n).caseDir{2} = ...
    'Ra3e5_r_c_0.01/00_2_iniTmp0_512/';
    configList(n).caseDir{3} = ...
    'Ra3e5_r_c_0.01/00_3_iniTmp0_1024/';
    configList(n).caseDir{4} = ...
    'Ra3e5_r_c_0.01/00_4_iniTmp0_1024/';
    configList(n).caseDir{5} = ...
    'Ra3e5_r_c_0.01/00_5_1024/';

    % n = n + 1;
    % configList(n).caseDir{1} = ...
    % 'Ra9e5/1_1_256/';
    
    % configList(n).caseDir{1} = ...
    % 'Ra3e6_r_c_0.01/04_1_Ra3e6_r_c_0.01_iniTmp0_1024/';
    % configList(n).caseDir{2} = ...
    % 'Ra3e6_r_c_0.01/04_2_Ra3e6_r_c_0.01_iniTmp0_1024/';
    % configList(n).caseDir{3} = ...
    % 'Ra3e6_r_c_0.01/04_3_Ra3e6_r_c_0.01_iniTmp0_1024/';
    % configList(n).caseDir{4} = ...
    % 'Ra3e6_r_c_0.01/04_4_Ra3e6_r_c_0.01_iniTmp0_1024/';
    % 
    % configList(n).caseDir{1} = ...
    % 'Ra3e7_r_c_0.01/02_1_initTmp0_1024/';
    % 
    % configList(n).caseDir{1} = ...
    % 'Ra3e8_r_c_0.01/03_1_iniTmp0_1024/';
    % 
    % configList(n).caseDir{1} = ...
    % 'Ra3e9_r_c_0.01/44_1_iniTmp0_1536/';
   

    for idxConfig = 1:numel(configList)
        for idxSubCase = 1:numel(configList(idxConfig).caseDir)
            configList(idxConfig).caseDir{idxSubCase} = ...
                fullfile(rootPath,configList(idxConfig).caseDir{idxSubCase});
        end
    end
end

