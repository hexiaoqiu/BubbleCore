function [k,sp_ener,sp_enst,sp_entropy,sp_buoy,flux_ener,flux_enst,flux_entropy,flux_buoy]=getSphFlux(bubble,timeStepStart,timeStepEnd)
    t0_debug = clock;
...
%     Ff = bubble.Ff;
    ...
    parfor time = timeStepStart:timeStepEnd
        ...
        imU_N = ImToPolarAcc(U_N,0,invMaxX2dS,n_half,n_cell);
        imTemp=ImToPolarAcc(temp,0,invMaxX2dS,n_half,n_cell);
...
%         imFTerm = Ff*(u_cent.^2 + v_cent.^2);
        ...
        imW=ImToPolarAcc(disque.*w',0,invMaxX2dS,n_half,n_cell);
        imU=ImToPolarAcc(disque.*u_cent',0,invMaxX2dS,n_half,n_cell);
        imV=ImToPolarAcc(disque.*v_cent',0,invMaxX2dS,n_half,n_cell);
        imTemp=ImToPolarAcc(disque.*temp',0,invMaxX2dS,n_half,n_cell);

        imres_u = ImToPolarAcc(disque.*res_u',0,invMaxX2dS,n_half,n_cell);
        imres_v = ImToPolarAcc(disque.*res_v',0,invMaxX2dS,n_half,n_cell);
        imres_w = ImToPolarAcc(disque.*res_w',0,invMaxX2dS,n_half,n_cell);
        imres_temp = ImToPolarAcc(disque.*res_temp',0,invMaxX2dS,n_half,n_cell);

        imBuoy=ImToPolarAcc(buoyancy',0,invMaxX2dS,n_half,n_cell);
        imres_buoy = ImToPolarAcc(res_buoy',0,invMaxX2dS,n_half,n_cell);
