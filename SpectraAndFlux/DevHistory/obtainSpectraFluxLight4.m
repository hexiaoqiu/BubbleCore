...
% This is the third version to compute spectra and flux!
% What's new:
% Added different method of window which is used to limit the spatial scope of the data for computation

function [k,sp_ener,sp_enst,sp_entropy,sp_buoy,flux_ener,flux_enst,flux_entropy,flux_buoy]=obtainSpectraFluxLight4(lightCase,t0,t1,logFile,varargin)
    t0_debug = clock;
   ...
   Sin_Theta = sin(acos(Z_pol));
    ...
    n1 = lightCase.n1;
    n2 = lightCase.n2;
    caseDir = lightCase.path;
    rootFileNameU = lightCase.rootFileNameU;
    rootFileNameV = lightCase.rootFileNameV;
    rootFileNameTmp = lightCase.rootFileNameTmp;
    t1_debug = clock;
    for time = t0:t1
        ...
        imU_N = ImToPolar_GPU(U_N,0,nn2,nn,n);
        imTemp=ImToPolar_GPU(temp,0,nn2,nn,n);
        ...
        imW=ImToPolar_GPU(disque.*w',0,nn2,nn,n);
        imU=ImToPolar_GPU(disque.*u_cent',0,nn2,nn,n);
        imV=ImToPolar_GPU(disque.*v_cent',0,nn2,nn,n);
        imTemp=ImToPolar_GPU(disque.*temp',0,nn2,nn,n);

        imres_u = ImToPolar_GPU(disque.*res_u',0,nn2,nn,n);
        imres_v = ImToPolar_GPU(disque.*res_v',0,nn2,nn,n);
        imres_w = ImToPolar_GPU(disque.*res_w',0,nn2,nn,n);
        imres_temp = ImToPolar_GPU(disque.*res_temp',0,nn2,nn,n);

        imBuoy=ImToPolar_GPU(buoyancy',0,nn2,nn,n);
        imres_buoy = ImToPolar_GPU(res_buoy',0,nn2,nn,n);
