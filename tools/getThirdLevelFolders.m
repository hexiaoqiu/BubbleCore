function allCaseDirList = getThirdLevelFolders(rootDir)
%GETTHIRDLEVELFOLDERS 获取指定目录下所有第三级文件夹的完整路径
%
% 输入：
%   rootDir - 根目录，例如：
...
%
% 输出：
%   allCaseDirList - 包含所有第三级文件夹完整路径的列 cell 数组。
%                    第三级文件夹自身名称包含 "readme"（忽略大小写）
%                    时会被排除；上级目录名称不参与判断。
