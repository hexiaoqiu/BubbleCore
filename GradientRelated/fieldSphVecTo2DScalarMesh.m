...
function [field2D] = fieldSphVecTo2DScalarMesh(fieldSph,M)
    

%     outValue = fieldSph(end);
%     outValue = 0;
%     outValue = fieldSph(end-1);
    ...
    numTheta = numel(fieldSph);
    thetaPositionVec = linspace(0,pi/2,numTheta)';
%     [field2D] = interp1(thetaPositionVec,fieldSph,thetaMatrix,'cubic',outValue);
    [field2D] = interp1(thetaPositionVec,fieldSph,thetaMatrix,"linear",'extrap');
