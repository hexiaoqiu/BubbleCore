function [success, structF_done] = calStructF_GPU2(varargin)
        ...
        for iD = 1:numD
            ...
            elseif strcmp(type,'U')
                ...
                for idx = 1:numel(timeStepList)
                    ...
                    f_OneStep = calVelOneStep(x2dU,y2dU,x2dV,y2dV,u2dGPU,v2dGPU, ...,
                    ...
                    % waitbar(idx/numel(timeStepList),h,sprintf('Latest time step: %d',idx)); 
                end
                % delete(h);
...
function [f] = calVelOneStep(x2dU,y2dU,x2dV,y2dV,u2dOrg,v2dOrg,p1_x2d,p1_y2d,p2_x2d,p2_y2d,itpMethod,order,tao_x_p1,tao_y_p1,tao_z_p1,tao_x_p2,tao_y_p2,tao_z_p2)
    ...
    for idx_order = 1:numel(order)
        f(idx_order) =  gather(mean( dVel.^(order(idx_order)), 'all'));
    end
end