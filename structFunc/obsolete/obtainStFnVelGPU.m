...
function [success] = obtainStFnVelGPU(varargin)
    ...
    for iD = 1:numD
        ...
        f_0 = gpuArray( zeros(1,numTimeStep) );
        t1 = clock;
        x2dU = gpuArray(StFn.x2dU);
        y2dU = gpuArray(StFn.y2dU);
        x2dV = gpuArray(StFn.x2dV);
        y2dV = gpuArray(StFn.y2dV);
        parfor iTime = 1:numTimeStep

            time = StFn.timeStepList(iTime);
            u2dOrg = gpuArray(readField(StFn,'u',time));
            v2dOrg = gpuArray(readField(StFn,'v',time));
        
            p1_u2d = interp2(x2dU, y2dU, u2dOrg, p1_x2d, p1_y2d, StFn.itpMethod);
            p1_v2d = interp2(x2dV, y2dV, v2dOrg, p1_x2d, p1_y2d, StFn.itpMethod);
            p2_u2d = interp2(x2dU, y2dU, u2dOrg, p2_x2d, p2_y2d, StFn.itpMethod);
            p2_v2d = interp2(x2dV, y2dV, v2dOrg, p2_x2d, p2_y2d, StFn.itpMethod);
